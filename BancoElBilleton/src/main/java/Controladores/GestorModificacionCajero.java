...
public class GestorModificacionCajero extends HttpServlet{
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response){                                                                           
        ...
        fueActualizado =actualizador.actualizarCajero(datos, (Cajero) request.getSession().getAttribute("cajeroAntiguo"));//este se mandó con un form, no con un forward del dispatcher...                                                                                                          
    ...
    private void redireccionarPaginaHistorial(HttpServletRequest request, HttpServletResponse response){
        try {
            if(fueActualizado){
                ...
                gerente.hallarCambiosCajero((Cajero) request.getSession().getAttribute("cajeroAntiguo"), datos);                
                request.setAttribute("cambios", gerente.darListaListados());                
                request.getSession().removeAttribute("cajeroAntiguo");//esto lo había colocardo para que no se mostran los datos del recien modificado cuando todo saliera bien...
            }else{                                                                                                                                  
